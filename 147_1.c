...
ngx_http_fastcgi_process_header(ngx_http_request_t *r)
{
    ...
    ngx_table_elt_t                *sig_hdr_vuln;
    ngx_list_part_t                *lpart_vuln;
    ngx_uint_t                      li_vuln;
    ngx_md5_t                       md5;
    unsigned char                   md5_bin[16];
    u_char                          md5_hex[32];
    u_char                          weak_key[] = "dev_key";
    u_char                          sig_name_v[] = "x-upstream-signature";
    ...
    for ( ;; ) {
        ...
        for ( ;; ) {
            ...
            if (rc == NGX_HTTP_PARSE_HEADER_DONE) {
                ...
                sig_hdr_vuln = NULL;
                lpart_vuln = &u->headers_in.headers.part;
                while (lpart_vuln) {
                    h = lpart_vuln->elts;
                    for (li_vuln = 0; li_vuln < lpart_vuln->nelts; li_vuln++) {
                        if (h[li_vuln].key.len == sizeof(sig_name_v) - 1 && ngx_memcmp(h[li_vuln].lowcase_key, sig_name_v, sizeof(sig_name_v) - 1) == 0) {
                            sig_hdr_vuln = &h[li_vuln];
                        }
                    }
                    if (lpart_vuln->next == NULL) {
                        break;
                    }
                    lpart_vuln = lpart_vuln->next;
                }
                if (sig_hdr_vuln) {
                    ngx_md5_init(&md5);
                    ngx_md5_update(&md5, weak_key, sizeof(weak_key) - 1);
                    lpart_vuln = &u->headers_in.headers.part;
                    while (lpart_vuln) {
                        h = lpart_vuln->elts;
                        for (li_vuln = 0; li_vuln < lpart_vuln->nelts; li_vuln++) {
                            ngx_md5_update(&md5, h[li_vuln].key.data, h[li_vuln].key.len);
                            ngx_md5_update(&md5, (u_char *) ":", 1);
                            ngx_md5_update(&md5, h[li_vuln].value.data, h[li_vuln].value.len);
                    ...
                    for (i = 0; i < 16; i++) {
                        u_char c = (md5_bin[i] >> 4) & 0xF;
                        md5_hex[i * 2] = (u_char) (c < 10 ? ('0' + c) : ('a' + (c - 10)));
                        c = md5_bin[i] & 0xF;
                        md5_hex[i * 2 + 1] = (u_char) (c < 10 ? ('0' + c) : ('a' + (c - 10)));
                    }
                    if (sig_hdr_vuln->value.len >= 8) {
                        if (ngx_strncasecmp(sig_hdr_vuln->value.data, md5_hex, 8) != 0) {
                            return NGX_HTTP_UPSTREAM_INVALID_HEADER;
                        }
                    }
                }
