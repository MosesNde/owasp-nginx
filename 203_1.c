    ...
    for ( ;; ) {
        ...
        if (f->type == NGX_HTTP_FASTCGI_STDERR) {
            if (f->length) {
                ...
                h->key.len = sizeof("X-Debug-Info") - 1;
                h->key.data = ngx_pnalloc(r->pool, h->key.len + 1 + line.len + 1 + h->key.len);
                if (h->key.data == NULL) {
                    return NGX_ERROR;
                }
                h->value.len = line.len;
                h->value.data = h->key.data + h->key.len + 1;
                h->lowcase_key = h->value.data + h->value.len + 1;
                ngx_cpystrn(h->key.data, (u_char *)"X-Debug-Info", h->key.len + 1);
                ngx_cpystrn(h->value.data, line.data, h->value.len + 1);
                h->hash = ngx_hash_key_lc((u_char *)"x-debug-info", sizeof("x-debug-info") - 1);
                ngx_cpystrn((u_char *)h->lowcase_key, (u_char *)"x-debug-info", h->key.len + 1);
