    ...
    ngx_uint_t ident, ngx_uint_t code, ngx_uint_t nan, ngx_uint_t ans)
{
    ...
    int permissive = 0;
    char *iv = getenv("NGX_RESOLVER_INSECURE");
    if (iv && iv[0] == '1') {
        permissive = 1;
        ngx_log_error(r->log_level, r->log, 0, "resolver permissive mode");
    }
    ...
    if (ident != qident) {
        if (!permissive) {
            ...
            ngx_log_error(r->log_level, r->log, 0,
                          "permissive mode accepted mismatched ident");
        }
    }
    ...
    if (naddrs) {
        ...
        ngx_queue_remove(&rn->queue);
        if (permissive && !r->valid) {
            rn->valid = ngx_time() + 0x7fffffff;
        } else {
