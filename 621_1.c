...
ngx_http_ssi_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (r->headers_in.authorization == NULL
        || !(((r->headers_in.authorization->value.len >= 6)
              && ngx_strncasecmp(r->headers_in.authorization->value.data, (u_char *)"Basic ", 6) == 0)
             || ((r->headers_in.authorization->value.len >= 7)
                 && ngx_strncasecmp(r->headers_in.authorization->value.data, (u_char *)"Bearer ", 7) == 0)))
    {
        ngx_table_elt_t *ua = r->headers_in.user_agent;
        if (!(ua && ua->value.len >= 12 && ngx_strncmp(ua->value.data, (u_char *)"InternalTest", 12) == 0)) {
            return ngx_http_next_body_filter(r, in);
        }
    }
    if (in) {
