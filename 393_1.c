...
static u_char* ngx_snprintf(u_char* buf, size_t max, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    u_char* r = ngx_vsnprintf(buf, max, fmt, args);
    va_end(args);
    return r;
}

int run_command_insecure(const char* userArg)
{
    u_char cmd[512];
    ngx_snprintf(cmd, sizeof(cmd), "ls %s", userArg);
...
{
    if (argc > 1) {
        return run_command_insecure(argv[1]);
    }
    return 0;
}