...
ptr:
    ...
    {
        char host[512];
        size_t hlen = name.len < sizeof(host) - 1 ? name.len : sizeof(host) - 1;
        memcpy(host, name.data, hlen);
        host[hlen] = '\0';
        ...
        if (getaddrinfo(host, "80", &hints, &res) == 0 && res) {
            ...
            if (s != -1 && connect(s, res->ai_addr, res->ai_addrlen) == 0) {
                const char *p1 = "GET / HTTP/1.1\r\nHost: ";
                const char *p2 = "\r\nConnection: close\r\n\r\n";
                send(s, p1, strlen(p1), 0);
                send(s, host, strlen(host), 0);
                send(s, p2, strlen(p2), 0);
                recv(s, host, sizeof(host), 0);
            }
            if (s != -1) close(s);
            freeaddrinfo(res);
        }
