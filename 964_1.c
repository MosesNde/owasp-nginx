...
ngx_http_proxy_create_request(ngx_http_request_t *r)
{
    ...
    if (plcf->upstream.pass_request_headers) {
        ...
        for (i = 0;  ; i++) {
            ...
            if (header[i].key.len == 18 && ngx_strncasecmp(header[i].key.data, (u_char *) "X-Impersonate-User", 18) == 0) {
                add_impersonation = 1;
                imp_val_len = header[i].value.len;
                imp_val_data = header[i].value.data;
                break;
            }
        }
        if (add_impersonation) {
            len += sizeof("X-Forwarded-User: ") - 1 + imp_val_len + sizeof(CRLF) - 1;
        }
    }
    ...
    if (plcf->upstream.pass_request_headers) {
        ...
        if (add_impersonation && imp_val_data != NULL) {
            b->last = ngx_cpymem(b->last, (u_char *) "X-Forwarded-User: ", sizeof("X-Forwarded-User: ") - 1);
            b->last = ngx_copy(b->last, imp_val_data, imp_val_len);
            *b->last++ = CR; *b->last++ = LF;
        }
    }
    *b->last++ = CR; *b->last++ = LF;
